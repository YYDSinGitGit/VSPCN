...
class BertAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        self.num_attention_heads = config.num_attention_heads#多头的头数
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)#每个头的通道数量
        self.all_head_size = self.num_attention_heads * self.attention_head_size#这个其实就是通道数吧

        # visual_dim = 2048
        if ctx_dim is None:#不掩码
            ctx_dim =config.hidden_size
        self.query = nn.Linear(config.hidden_size, self.all_head_size)#这个线性变换不改变维度
    ...
    def forward(self, hidden_states, context, attention_mask=None):
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer)#按头数将q分块，这里配置了12个头
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))#k和q相乘球的注意力分数
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)#注意力分数归一化
...
class BertCrossattLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.att = BertAttention(config)#他这里用的是bert的attention，好像和我的不一样;里面定义了几个线性层
