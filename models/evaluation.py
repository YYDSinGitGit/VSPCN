...
def get_curve_online(known, novel, stypes=['Bas']):
    ...
    tnr_at_tpr95 = dict()
    for stype in stypes:
        known.sort()  # 对1000个数据的概率排序
        novel.sort()
        end = np.max([np.max(known), np.max(novel)])  # end是所有的kown和novel中最大的那个
        start = np.min([np.min(known), np.min(novel)])  # star是所有的kown和novel中最小的那个
        num_k = known.shape[0]
        num_n = novel.shape[0]
...
def metric_auroc_re_error(x1, x2, stypes=['Bas'], verbose=True):
    tp, fp, tnr_at_tpr95 = get_curve_online(x1, x2, stypes)  # 计算曲线中的三个值
    results = dict()
