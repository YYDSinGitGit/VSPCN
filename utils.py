...
def compute_class_accuracy_total(true_label, predict_label, args):
    acc_per_class = np.zeros(args.num_seen)
    for i in range(args.num_seen):
...
def compute_class_accuracy_total1(true_label, true_label1, predict_label, args):
    acc_per_class = np.zeros(args.num_seen)  # 共有num_seen个可见类别
    for i in range(args.num_seen):
        idx = np.where(true_label == i)[0]  # 全部类别i的真实标签
        idx1 = np.where(true_label1 == i)[0]  # 过滤后类别i的真实标签
        acc_per_class[i] = (sum(true_label1[idx1] == predict_label[idx1]) * 1.0 / len(
            idx))  # 计算过滤后i类别的精度，注意：这里分母应该是len(idx)【i类别的总数】，不是len(idx1)【这是过滤后的i类总数】
...
def post_process(v_prob, a_prob, gt, split_num, seen_c, unseen_c, data):
    ...
    H_v = entropy(v_prob)  # odr框架输出的熵值
    v_pre = np.argmax(v_prob, axis=1)  # odr标签

    a_max = np.max(v_prob, axis=1)  # 这里有没有写错，不应该是a_prob吗，为什么是v_prob
    H_a = entropy(a_prob)
    a_pre = np.argmax(a_prob, axis=1)  # zsl标签
    ...
    for step in range(9):  # 找一个合适的step
        base = 0.1 * step + 0.1
        tau = -base * np.log(base)
        pre = v_pre  # odr的标签
        for idx, class_i in enumerate(pre):
            if (v_max[idx] - base < 0):  # 如果odr对测试数据的置信度不够，则该数据采用zsl的输出概率
                pre[idx] = a_pre[idx]
        ...
        Ds = compute_domain_accuracy(pre_s, seen_c)  # 这两个指标应该没有用吧
        Du = compute_domain_accuracy(pre_t, unseen_c)  # 这两个指标应该没有用吧
        H = 2 * S * U / (S + U)
...
def adjust_learning_rate(optimizer,epoch, args):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    # lr = args.lr * (0.5 ** (epoch // args.epoch_decay))#微调用
    lr = args.lr *(args.epoch_decay_rate** (epoch // args.epoch_decay))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
